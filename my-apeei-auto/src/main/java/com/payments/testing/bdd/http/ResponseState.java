...
import com.payments.testing.bdd.parameters.ParamTransformer;
...
public abstract class ResponseState {
  ...
  /**
   * Assert provide path value is greater than the expected value
   * @param path the path to extract the value from
   * @param obj the value to match against
   */
  public void greaterThan(String path, Integer obj) {
	    Integer val = Integer.valueOf(getValue(path).toString());
	    assertTrue(val>obj);
	  }
  
  /**
   * Assert provide path value is less than the expected value
   * @param path the path to extract the value from
   * @param obj the value to match against
   */
  public void lessThan(String path, Integer obj) {
	    Integer val = Integer.valueOf(getValue(path).toString());
	    assertTrue(val<obj);
	  }
